...
bh_partition <- createDataPartition(BostonHousing$medv, p = 0.8, list = FALSE)
...
lm_model <- train(medv ~ ., data = bh_train, method = "lm")
y_pred_lm <- predict(lm_model, bh_test[ , 1:13])

cor(y_pred_lm, bh_test[ , 14])
lm_model # note RMSE, MAE
...
# Split into train and test data
pi_partition <- createDataPartition(PimaIndiansDiabetes$diabetes, p = 0.8, list = FALSE)
...
logit_model <- train(factor(diabetes) ~ ., data = pi_train, method = "glm", family = "binomial")
...
y_pred_nn_class <- predict(nn_class_model, pi_x_test) %>% k_argmax() %>% as.matrix %>% as.factor
pi_y_test <- pi_y_test %>% k_argmax() %>% k_cast("int32") %>% as.matrix %>% as.factor
